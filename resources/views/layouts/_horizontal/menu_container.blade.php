    {{-- menu_container.blade.php - Horizontal Menu Container - Backpack For Laravel Admin Panel
...
<header data-bs-theme={{ $theme ?? 'system' }} class="{{ backpack_theme_config('classes.menuHorizontalContainer') ?? 'flex flex-col top' }}">
    <div class="flex flex-col" id="navbar-menu">
        <div class="print:hidden {{ backpack_theme_config('classes.menuHorizontalContent') ?? 'flex flex-col '.($theme ?? 'light').' '.(($overlap ?? false) ? 'overlap' : '') }}">
...
<aside data-bs-theme={{ $theme ?? 'system' }} class="flex flex-col {{ $theme ?? 'light' }} block lg:hidden">
    <div class="container mx-auto">
        ...
        <div class="flex flex-col" id="mobile-menu">
