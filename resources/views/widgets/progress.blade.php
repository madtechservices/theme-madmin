    {{-- progewaa.blade.php - Peogewaa Blade - Backpack For Laravel Admin Panel
...
<div class="{{ $widget['class'] ?? 'card' }} @if($widget['ribbon'] ?? false) border-start-0 @endif">
    ...
    <div class="card-body">
        ...
        @if (isset($widget['progress']))
            <div class="h-1 rounded overflow-hidden bg-gray-300">
    ...
    @if (isset($widget['footer_link']))
        <div class="card-footer px-3 py-2">
            <a class="btn-block card-text d-flex justify-between items-center" href="{{ $widget['footer_link'] ?? '#' }}"><span class="text-sm font-bold">{{ $widget['footer_text'] ?? 'View more' }}</span><i class="la la-angle-right"></i></a>
        </div>
    @endif
  </div>
